...
const CONFIG = {
  ...
  STAFF: [
    ...
    { name: '三島圭織', role: 'staff', start: '10:00', end: '16:00', breakMin: 60, offDays: [3], pattern: '水休み', company: 'クリアメンテ', showTimeline: false },
  ],
  ...
  CHANGELOG: [
    {
      ...
      notify: true,
      changes: [
        { type: '変更', text: 'ボトムナビを作業向けに刷新（ホーム/チャット/分荷判定/出荷登録/取引ナビ）' },
        { type: '新機能', text: '取引ナビ：スクショでステータス自動更新 + 売上登録' },
        { type: '新機能', text: 'PIN認証（4桁暗証番号）でログイン保護' },
        { type: '新機能', text: '保管場所を階層化（拠点→厚見A1-28/倉庫奥/2階→フリー追記）' },
        { type: '新機能', text: '実測採寸を全商品で入力可能（フリー形式）' },
        { type: '新機能', text: '分荷判定後に管理番号を常時表示' },
        { type: '改善', text: '撮影は1枚から判定OK。追加は任意' },
        { type: '改善', text: '商品名をタップで手動修正可能（AI上書き防止）' },
        { type: '改善', text: 'ブラウザ戻るボタンでアプリ内ステップを戻る' },
        { type: '改善', text: '作業途中データがタブを閉じても保持される' },
        { type: '変更', text: 'Google Chatに通知（相談→通販業務、売上→売上明細）' },
        { type: '新機能', text: '再判定ボタン（理由入力付き）' },
        { type: '変更', text: '右上アイコンがマイページに（アバター画像反映）' },
      ],
    },
    {
      date: '2026-04-14',
